...
static const int STAKE_START_TIME = 1464393600;  // Staking Begins At 05/28/2016 @ 12:00am (UTC)
static const int STAKE_TARGET_SPACING = 300;
static const int STAKE_MIN_AGE = 3600;
static const int STAKE_MAX_AGE = 60 * 60 * 24 * 1000;
...
public:
    ...
    {
        // EGGS: the coin stake transaction is marked with the first output empty
    ...
    */
    ...
    bool GetCoinAge(CTxDB& txdb, uint64& nCoinAge) const;  // EGGS: get transaction coin age
...
public:
    ...
    // EGGS: block signature - signed by coin base txout[0]'s owner
    ...
    // EGGS: two types of block: proof-of-work or proof-of-stake
    ...
    // EGGS: get max transaction timestamp
    ...
    bool AcceptBlock();
    bool GetCoinAge(uint64& nCoinAge) const; // EGGS: calculate total coin age spent in block
    bool SignBlock(const CKeyStore& keystore);
    bool CheckBlockSignature() const;
    unsigned int GetStakeEntropyBit() const; // EGGS: entropy bit for stake modifier if chosen by modifier
...
public:
    ...
    unsigned int nFile;
    unsigned int nBlockPos;
    CBigNum bnChainTrust; // EGGS: trust score of block chain
    int nHeight;
    int64 nMint;
    int64 nMoneySupply;

    unsigned int nFlags;  // EGGS: block index flags
    enum  
    {
