...
// EGGS: optional setting to unlock wallet for block minting only;
...
// EGGS: total coins staked (non-spendable until maturity)
       ...
       BOOST_FOREACH(const CWalletTx* pcoin, vCoins)
       {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++)
            {
                ...
                if (pcoin->nTime > nSpendTime)
                    continue;  // EGGS: timestamp must not exceed spend time
...

    {
        ...
        CTxDB txdb("r");
        {
            ...
            while (true)
            {
                ...
                // EGGS: sub-cent change is moved to fee
                ...
                if (nChange > 0)
                {
                    ...
                    else
                    {
                        ...
                        if (!GetBoolArg("-avatar")) // EGGS: not avatar mode
                        {
...
// EGGS: create coin stake transaction
...
// EGGS: check 'spent' consistency between wallet and txindex
// EGGS: fix wallet spent state according to txindex
...
// EGGS: disable transaction (only for coinstake)
